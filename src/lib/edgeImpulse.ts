...
/**
 * Extra key/values attached to the EI ingestion `x-metadata` header.
 * Canonical definition lives in `./types`; re-exported here so dependents
 * can import it from either place.
 */
export type { IngestionMetadataExtras } from './types';

/**
 * Per-project Edge Impulse upload configuration. Originally owned by the
 * store; defined here in the PlayCanvas port so the pure upload lib stays
 * store-agnostic — the store/UI layer should import it from this module.
 */
...
export async function buildDataAcquisitionPayload(
  ...
  // engine update tick), so 100 Hz requested + 60 fps render = ~60 Hz
  // actual, and a 2 s recording would render as 1.2 s if we naively
  // reported 1000/100 = 10 ms here. Prefer the actual per-sample spacing
  // measured from the recorded timestamps; fall back to the requested
  // rate only when there aren't enough samples to derive it.
