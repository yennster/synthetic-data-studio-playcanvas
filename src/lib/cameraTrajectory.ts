import type { CameraTrajectory } from './types';
 ...
 * lands.
 *
 * Pure math — no renderer types. Engine hook point: the PlayCanvas
 * capture loop feeds the returned tuple into its capture entity's
 * `setPosition(x, y, z)` + `lookAt(target)` each batch iteration.
 *
 * Trajectories:
