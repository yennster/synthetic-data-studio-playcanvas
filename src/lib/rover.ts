 ...
 * the whole traversal, and the contact detector watches the rover's
 * chassis disc against the obstacle list each frame — it produces the
 * actual contact events that feed the IMU impulse and the recorded
 * label.
 */

// Shared with urlParams + the store via ./types — the dependency-free
// home for the persisted RoverEvent labels.
import type { RoverEvent } from './types';
...
function buildStuckPath(
  ...
  // ~5 cm of static overlap to keep the contact check firing every tick.
