 ...
 *
 * Engine hook point: the PlayCanvas IMU sampler reads a clean
 * (accel, gyro) pair from the physics body each sample tick, threads
 * its per-sensor `ImuNoiseState` through `applyImuNoise` with the
 * actual accumulated sample period as `dt`, and emits the result as
 * an `AccelSample`. State must be recreated on body/shape remount so
 * each recording gets its own drift trajectory.
 */
