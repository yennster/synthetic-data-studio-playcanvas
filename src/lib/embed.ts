...
//
// All `apply*FromUrl` helpers are pure over their inputs: the store /
// theme setter is injected by the caller, so the actual store wiring
// happens in the app layer (main.tsx bootstrap), not here.
...
// Re-exported so existing `import { EiCategory } from './embed'` callers
// keep working — the union now lives in shared ./types.
export type { EiCategory };
