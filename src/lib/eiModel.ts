...
class EdgeImpulseClassifier {
  ...
  // Explicit field (not a constructor parameter property) — the rebuild's
 ...
 *
 * Engine hook point: the caller supplies any 2D-drawable canvas. In the
 * PlayCanvas rebuild that's the preview readback canvas (the engine copies
 * its render-target pixels into a plain <canvas> before inference), so this
 * stays a pure DOM function with no engine dependency.
 */
