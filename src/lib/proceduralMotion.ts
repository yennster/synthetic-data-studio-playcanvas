/**
 * Pure helpers used by the procedural-motion runner (drop / throw /
 * push / shake motion classes). Extracted here so the timing logic can
 * be unit-tested without booting the whole zustand store + scene.
 *
 * Engine hook point: the runner itself lives in the UI layer — it
 * lifts the kinematically-held body to a random pose, starts
 * recording, sleeps `randomPreReleaseMs`, then releases / oscillates
 * the body via the physics engine and snapshots the IMU samples.
 */
