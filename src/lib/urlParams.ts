 ...
 *     batch count, EI label, …). Parsed eagerly here into typed values;
 *     the store wiring happens in the app layer (`applyUrlPresets` is
 *     rebuilt there) — this module stays a pure parser so it can be
 *     unit-tested without booting the store or the engine.
 *
...
import type { CameraTrajectory, RoverEvent } from './types';

// These unions lived on the zustand store (and useTheme) in the original
// three.js app. Re-declared here so lib/ stays store-agnostic; the store
// re-uses them via this module (or its own mirror) in the app layer.
