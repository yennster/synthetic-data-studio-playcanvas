...
describe('detectPlatform', () => {
  ...
  it('treats a desktop Mac Safari UA as macOS-12+ eligible', () => {
    const p = detectPlatform(MAC_SAFARI, 0);
    expect(p.os).toBe('mac');
    expect(p.macosMajor).toBe(12);
    expect(p.supportsObjectCaptureMac).toBe(true);
    expect(p.supportsObjectCaptureMobile).toBe(false);
    expect(p.isMobile).toBe(false);
  });

  it('treats a desktop Mac Chrome UA as macOS-12+ eligible', () => {
    const p = detectPlatform(MAC_CHROME, 0);
    expect(p.os).toBe('mac');
    expect(p.supportsObjectCaptureMac).toBe(true);
    expect(p.supportsObjectCaptureMobile).toBe(false);
  });
  ...
  it('returns "other" for non-Apple platforms with no Object Capture support', () => {
    for (const ua of [WINDOWS, ANDROID]) {
      ...
      expect(p.iosMajor).toBeNull();
      expect(p.macosMajor).toBeNull();
