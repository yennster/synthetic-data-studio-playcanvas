...
/**
 * Decide what should happen when the user types `next` into a field
 * whose upstream value is currently `value`. Empty / lone-minus /
 * unparseable drafts are preserved without committing; finite numbers
 * are clamped and committed only when they differ from the current
 * upstream value.
 */
...
/**
 * Decide what should happen when the user blurs out of the field.
 * Empty / lone-minus / unparseable drafts snap back to the last
 * committed value; out-of-range finite numbers are clamped and
 * committed.
 */
...
/**
 * Controlled `<input type="number">` state that tolerates a transiently
 * empty or otherwise invalid string while the user is typing.
 *
 * The naive pattern — `value={n}` +
 ...
 * can never see it empty: clearing "10" snaps straight back to "10"
 * before the next digit can be typed.
 *
 * This hook keeps a local `draft` string mirroring what's actually in
 ...
 * keystroke commits and blur; out-of-range typed values are committed
 * at the boundary (e.g. typing "9999" with `max: 500` commits 500).
 *
...
/**
 * Drop-in `<input type="number">` using `useNumberInput` under the
 * hood — also usable inside `.map()` loops where calling the hook
 * directly per row would violate the rules of hooks.
 *
 * With `label` set it renders a full labeled field row (same look as
 * SliderRow: uppercase micro-label above the input); without it, just
 * the bare styled input for inline layouts. `min`/`max` both clamp
 * committed values and are forwarded to the DOM element along with
 * `step`.
 */
export function NumberField(props: {
  value: number;
  onChange: (n: number) => void;
  /** Optional field label; when set, wraps the input in a labeled row. */
  label?: ReactNode;
  min?: number;
  max?: number;
  step?: number;
  disabled?: boolean;
  placeholder?: string;
  /** Native `title` tooltip (on the row when labeled, else the input). */
  title?: string;
  style?: CSSProperties;
  className?: string;
  'aria-label'?: string;
  ...
  const input = (
    <input
      type="number"
      className={`ui-number${props.className ? ` ${props.className}` : ''}`}
      min={props.min}
      max={props.max}
      step={props.step}
      ...
      style={props.style}
      title={props.label == null ? props.title : undefined}
      aria-label={props['aria-label']}
      {...inputProps}
    />
  );
  if (props.label == null) return input;
  return (
    <label
      className={`ui-field${props.disabled ? ' disabled' : ''}`}
      title={props.title}
    >
